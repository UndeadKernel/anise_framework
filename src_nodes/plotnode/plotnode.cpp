...
void CPlotNode::configure(CNodeConfig &config)
{
    ...
    config.addBool("gaussian_data", "Plots for gaussian data",
                   "Can plot gaussian curves for GMM, else will be a 1D scatter plot",
                   true);
    config.addString("output_filename", "Name of the png file",
...
bool CPlotNode::data(QString gate_name, const CConstDataPointer &data)
{
    ...
    if(data->getType() != "table") {
        // We only process table data.
        return false;
    }

    // User parameters
    bool gaussian_data = getConfig().getParameter("gaussian_data")->value.toBool();
    QString output_filename = getConfig().getParameter("output_filename")->value.toString();
